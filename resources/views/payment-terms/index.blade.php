        ...
        <table class="min-w-full divide-y divide-blue-200">
          ...
          @foreach($PaymentTerms as $PaymentTerm)
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                @if($PaymentTerm->status == 0)
                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                    Disabled
                    </span>
                    @elseif($PaymentTerm->status == 1)
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    <button x-data="{usedKeyboard: false}"
			@keydown.window.tab="usedKeyboard = true"
			@click="$dispatch('open-menu', { open: true,id: {{ $PaymentTerm->id }} })"
			:class="{'focus:outline-none': !usedKeyboard}" type="button" class="relative inline-flex items-center px-4 py-2 rounded-l-md border border-green-700 bg-green-600 text-sm font-medium text-white hover:bg-green-500 focus:z-10 focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500">
                    ...
                    <form action="{{ route('payment-terms.delete', $PaymentTerm->id) }}" method="DELETE" onsubmit="return confirm('Are you sure want to delete Payment Terms?')">
                      @csrf @method('DELETE')
...
<section
  ...
  @keydown.escape="open = false"
	...
	<div
        ...
        <form class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll" action="{{ route('payment-terms.update') }}" method="post">
          <input type="hidden" value="" name="id" id="payment_term_id"/>
          @csrf
          <div class="flex-1">
            <!-- Header -->
            ...
            <div class="py-6 space-y-6 sm:py-0 sm:space-y-0 sm:divide-y sm:divide-gray-200">
              ...
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                <div>
                  <label for="payment_term_name_label" class="block text-sm font-medium text-gray-900 sm:mt-px sm:pt-2">
                    Name
                  </label>
                </div>
                <div class="sm:col-span-2">
                  <input type="text" name="name" id="payment_term_name" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <!-- PaymentTerm Abrv -->
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                <div>
                  <label for="payment_term_abrv_label" class="block text-sm font-medium text-gray-900 sm:mt-px sm:pt-2">
                    Abrv
                  </label>
                </div>
                <div class="sm:col-span-2">
                  <input type="text" name="abrv" id="payment_term_abrv" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <!-- PaymentTerm Days -->
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                <div>
                  <label for="payment_term_days_label" class="block text-sm font-medium text-gray-900 sm:mt-px sm:pt-2">
                    Days
                  </label>
                </div>
                <div class="sm:col-span-2">
                  <input type="text" name="days" id="payment_term_days" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <!-- PaymentTerm Status -->
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                ...
                <div class="sm:col-span-2">
                  <input type="checkbox" name="status" id="payment_term_status" class="form-checkbox h-5 w-5 text-blue-600"><span class="ml-2 text-gray-700">
                </div>
              </div>
...
<section
  ...
  @keydown.escape="createsection = false"
	...
	<div
		...
		<div class="p-16 px-6 absolute top-0 h-full overflow-y-scroll">
		<div class="w-screen max-w-2xl">
        <form class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll" action="{{ route('payment-terms.store') }}" method="post">
          @csrf
          <div class="flex-1">
            <!-- Header -->
            <div class="px-4 py-6 bg-gray-50 sm:px-6">
...
function getPaymentTermDetail(id){
  var url = '{{ route("payment-terms.edit", ":id") }}';
  ...
  $.ajax({
      ...
      if(response.status == 'success'){
        $("#payment_term_name").val(response.PaymentTerms.name);
        $("#payment_term_id").val(response.PaymentTerms.id);
        $("#payment_term_abrv").val(response.PaymentTerms.abrv);
        $("#payment_term_days").val(response.PaymentTerms.days);
        if(response.PaymentTerms.status == 1)
        $("#payment_term_status").prop('checked',true);
      }
    }
 });
}
