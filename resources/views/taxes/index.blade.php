        ...
        <table class="min-w-full divide-y divide-blue-200">
          ...
          @foreach($Taxs as $Tax)
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <span class="relative z-0 inline-flex shadow-sm rounded-md">
                    <button x-data="{usedKeyboard: false}"
			@keydown.window.tab="usedKeyboard = true"
			@click="$dispatch('open-menu', { open: true,id: {{ $Tax->id }} })"
			:class="{'focus:outline-none': !usedKeyboard}" type="button" class="relative inline-flex items-center px-4 py-2 rounded-l-md border border-green-700 bg-green-600 text-sm font-medium text-white hover:bg-green-500 focus:z-10 focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-green-500">
                    ...
                    <form action="{{ route('taxes.delete', $Tax->id) }}" method="DELETE" onsubmit="return confirm('Are you sure want to delete Tax?')">
                      @csrf @method('DELETE')
...
<section
  ...
  @keydown.escape="open = false"
	...
	<div
        ...
        <form class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll" action="{{ route('taxes.update') }}" method="post">
          ...
          <div class="flex-1">
            ...
            <div class="py-6 space-y-6 sm:py-0 sm:space-y-0 sm:divide-y sm:divide-gray-200">
              ...
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                <div>
                  <label for="tax_name_label" class="block text-sm font-medium text-gray-900 sm:mt-px sm:pt-2">
                    Name
                  </label>
                </div>
                <div class="sm:col-span-2">
                  <input type="text" name="name" id="tax_name" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <!-- Tax Value -->
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                <div>
                  <label for="tax_value_label" class="block text-sm font-medium text-gray-900 sm:mt-px sm:pt-2">
                    Value
                  </label>
                </div>
                <div class="sm:col-span-2">
                  <input type="text" name="value" id="tax_value" class="block w-full shadow-sm sm:text-sm focus:ring-indigo-500 focus:border-indigo-500 border-gray-300 rounded-md">
                </div>
              </div>
              <!-- Tax Status -->
              <div class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 sm:py-5">
                ...
                <div class="sm:col-span-2">
                  <input type="checkbox" name="status" id="tax_status" class="form-checkbox h-5 w-5 text-blue-600"><span class="ml-2 text-gray-700">
                </div>
              </div>
...
<section
  ...
  @keydown.escape="createsection = false"
	...
	<div
		...
		<div class="p-16 px-6 absolute top-0 h-full overflow-y-scroll">
		<div class="w-screen max-w-2xl">
        <form class="h-full flex flex-col bg-white shadow-xl overflow-y-scroll" action="{{ route('taxes.store') }}" method="post">
          @csrf
          <div class="flex-1">
            <!-- Header -->
            <div class="px-4 py-6 bg-gray-50 sm:px-6">
...
function getTaxDetail(id){
  ...
  $.ajax({
      ...
      if(response.status == 'success'){
        $("#tax_name").val(response.Taxs.name);
        $("#tax_value").val(response.Taxs.value);
        $("#tax_id").val(response.Taxs.id);
        if(response.Taxs.status == 1)
